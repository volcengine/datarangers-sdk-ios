...
@interface BDAutoTrackDatabase ()

- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation BDAutoTrackDatabase
@synthesize cachedStatements=_cachedStatements;

#pragma mark BDAutoTrackDatabase instantiation and deallocation
...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
    ...
    return YES;
#else
    //Log(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    BDAutoTrackDatabase *self = (__bridge BDAutoTrackDatabase*)f;
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            //Log(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
// but for folks who don't bother noticing that the interface to BDAutoTrackDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    //Log(@"%s:%d", __FUNCTION__, __LINE__);
    //Log(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    //Log(@"%s:%d", __FUNCTION__, __LINE__);
    //Log(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        BDAutoTrackResultSet *rs = (BDAutoTrackResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(BDAutoTrackResultSet *)resultSet {
...
- (BDAutoTrackStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(BDAutoTrackStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(BDAutoTrackStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    BDAutoTrackResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    //Log(@"The BDAutoTrackDatabase %@ is currently in use.", self);
}
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"BDAutoTrackDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    BDAutoTrackStatement *statement  = 0x00;
    BDAutoTrackResultSet *rs         = 0x00;
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                //Log(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
    ...
    if (idx != queryCount) {
        //Log(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
    ...
    if (!statement) {
        statement = [[BDAutoTrackStatement alloc] init];
    ...
    rs = [BDAutoTrackResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (BDAutoTrackResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    BDAutoTrackResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    BDAutoTrackStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[BDAutoTrackStatement alloc] init];
...
static int BDAutoTrackDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
static int BDAutoTrackDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? BDAutoTrackDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
#else
    NSString *errorMessage = NSLocalizedString(@"Save point functions require SQLite 3.7", nil);
    return [NSError errorWithDomain:@"BDAutoTrackDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
...
static void BDAutoTrackDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
static void BDAutoTrackDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &BDAutoTrackDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &BDAutoTrackDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation BDAutoTrackStatement
