...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `BDAutoTrackDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<BDAutoTrackResultSet>` - Represents the results of executing a query on an `BDAutoTrackDatabase`.
 - `<BDAutoTrackDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<BDAutoTrackStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `BDAutoTrackDatabase` object and use it across multiple threads. Instead, use `<BDAutoTrackDatabaseQueue>`.
...
@interface BDAutoTrackDatabase : NSObject  {
...
/** Create a `BDAutoTrackDatabase` object.
 
 An `BDAutoTrackDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `BDAutoTrackDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `BDAutoTrackDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    BDAutoTrackDatabase *db = [BDAutoTrackDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    BDAutoTrackDatabase *db     = [BDAutoTrackDatabase databaseWithPath:dbPath];
 ...
 @return `BDAutoTrackDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `BDAutoTrackDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `BDAutoTrackDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
/** Execute select statement
 ...
 Executing queries returns an `<BDAutoTrackResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[BDAutoTrackResultSet next]>`) from one record to the other.
 ...
 @return A `<BDAutoTrackResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see BDAutoTrackResultSet
 @see [`BDAutoTrackResultSet next`](<[BDAutoTrackResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `BDAutoTrackDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (BDAutoTrackResultSet *)executeQuery:(NSString*)sql, ...;
...
- (BDAutoTrackResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String!, values: [AnyObject]!) throws  -> BDAutoTrackResultSet!`
...
- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error;
...
- (BDAutoTrackResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (BDAutoTrackResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(BDAutoTrackDatabase *adb) {
        ...
        int rowCount = 0;
        BDAutoTrackResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [BDAutoTrackDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using BDAutoTrackDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `BDAutoTrackStatement`, but rather with `<BDAutoTrackDatabase>` and `<BDAutoTrackResultSet>` only.
 
 ### See also
 
 - `<BDAutoTrackDatabase>`
 - `<BDAutoTrackResultSet>`
 - [`sqlite3_stmt`](http://www.sqlite.org/c3ref/stmt.html)
 */

@interface BDAutoTrackStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
