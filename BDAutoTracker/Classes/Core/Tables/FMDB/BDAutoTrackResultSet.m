...
@implementation BDAutoTrackResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(BDAutoTrackStatement *)statement usingParentDatabase:(BDAutoTrackDatabase*)aDB {
    
    BDAutoTrackResultSet *rs = [[BDAutoTrackResultSet alloc] init];
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        //Log(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        //Log(@"Database busy");
        if (outErr) {
        ...
        //Log(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"BDAutoTrackDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
        ...
        // wtf?
        //Log(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (void)setParentDB:(BDAutoTrackDatabase *)newDb {
    _parentDB = newDb;
}
